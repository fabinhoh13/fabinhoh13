...
contaUmAberto :: Parser Char Int
contaUmAberto = f <$> symbol '('
  where
    f c = 1

descartaFechado :: Parser Char Int
descartaFechado = f <$> symbol ')'
  where
    f c = 0

descartaMinusculo :: Parser Char Int
descartaMinusculo = f <$> (symbol 'a' <|>
                       symbol 'b' <|>
                       symbol 'c' <|>
                       symbol 'd' <|>
                       symbol 'e' <|>
...
descartaMaiusculo :: Parser Char Int
descartaMaiusculo = f <$> (symbol 'A' <|>
                       symbol 'B' <|>
                       symbol 'C' <|>
                       symbol 'D' <|>
                       symbol 'E' <|>
...
descartaNumero :: Parser Char Int
descartaNumero = f <$> (symbol '0' <|>
                       symbol '1' <|>
                       symbol '2' <|>
                       symbol '3' <|>
                       symbol '4' <|>
...
descartaSimbolo :: Parser Char Int
descartaSimbolo = f <$> (symbol '+' <|>
                       symbol '-' <|>
                       symbol '_' <|>
                       symbol '=' <|>
                       symbol '*' <|>
...
pairs :: Parser Char Int
pairs = foldl f 0 <$> greedy (contaUmAberto <|>
                              descartaFechado <|>
                              descartaMinusculo <|>
                              descartaMaiusculo <|>
                              descartaNumero <|>
...
naturalSub :: Parser Char Int
naturalSub = foldl f 0 <$> many digit
  where
    f ac d = ac * 10 - d

integer :: Parser Char Int
integer = ( (\ x y -> y) <$> (symbol '-') <*> naturalSub) <|> natural
