...
filterListC _ [] = []
filterListC f list = [x | x <- list, (f x)]
...
intervalWFilter :: [Int] -> Int -> Int -> [Int]
intervalWFilter list a b = filter (interval a b) list
...
intervalWRecursion :: [Int] -> Int -> Int -> [Int]
intervalWRecursion [] _ _ = []
intervalWRecursion (x:xs) a b = 
  if (x >= a && x <= b)
    then x:(intervalWRecursion xs a b)
    else (intervalWRecursion xs a b)
...
sumQuad :: [Int] -> Int
sumQuad = sum . map (\x -> x^2) . filter odd
...
allRec f [] = True
allRec f l = allRec' f l True
  where
    allRec' f [] a = a
    allRec' f [x] a allRec' f [] ((f x) && a)
    allRec' f (x:xs) a = allRec' f xs ((f x) && a)
\end{code}
